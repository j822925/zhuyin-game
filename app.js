import {BASE,COMPOUNDS,normalizeConfig,createCatalog,poolFor,optionsFor,shuffle,questionDeck,Round} from './core.js?v=20260913-stars4';
import {createMultiplayer} from './multiplayer.js?v=20260913-stars4';
import {createRewards} from './rewards.js?v=20260913-stars4';
import {portrait} from './characters.js?v=20260913-stars4';
import {setupChildUI} from './child-ui.js?v=20260913-stars4';
import {setupCozyUI} from './cozy-ui.js?v=20260913-stars4';
import {createStudentAuth} from './student-auth.js?v=20260913-stars4';
import {createApiClient} from './api-client.js?v=20260913-stars4';
...
async function load(){
 ...
 try{catalog=createCatalog(await fetch('data/syllables.json').then(r=>{if(!r.ok)throw new Error();return r.json();}));
  config=normalizeConfig(demo?{version:2,symbols:['ㄅ','ㄆ','ㄇ','ㄉ','ㄧ','ㄠ'],compounds:[],seats:Array.from({length:15},(_,i)=>String(i+1)),questions:10,spellingApproved:true}:await jsonGet('?api=config'));
...
function renderQuestion(){
 const r=rounds[active],q=r.current;r.questionStarted=Date.now();selected={};audio.pause();audio.src=q.audio;audioReady=false;
 ...
 else{document.querySelectorAll('.slot').forEach(b=>{b.textContent=b.dataset.slot==='initial'?'聲符':'韻符';b.classList.remove('filled');});$('tiles').replaceChildren();for(const kind of ['initial','final']){const row=document.createElement('div');row.className='tile-row';const target=q[kind],other=[...new Set(currentPool.map(x=>x[kind]))].filter(x=>x!==target);for(const value of shuffle([target,...shuffle(other).slice(0,choices-1)])){const b=document.createElement('button');b.className='tile';b.textContent=value;b.dataset.kind=kind;b.draggable=true;b.onclick=()=>place(kind,value);b.ondragstart=e=>e.dataTransfer.setData('text/plain',JSON.stringify({kind,value}));b.onpointerup=e=>{if(e.pointerType==='mouse')return;const slot=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-slot]');if(slot?.dataset.slot===kind)place(kind,value);};row.append(b);}$('tiles').append(row);}}
 setAnswerEnabled(false);if(duo)teamUI.turnPrompt(active,seats,()=>{r.questionStarted=Date.now();play();});else play();
}
function place(kind,value){if(!audioReady||rounds[active].locked)return;if(!currentPool.some(x=>x[kind]===value)||!['initial','final'].includes(kind))return;selected[kind]=value;const b=document.querySelector(`[data-slot="${kind}"]`);b.textContent=value;b.classList.add('filled');$('check-spelling').disabled=!selected.initial||!selected.final;}
...
function finish(){audio.pause();teamUI.stop();screen('result');let html='';const results=[];
 ...
 $('result-details').innerHTML=html;document.querySelectorAll('[data-review]').forEach(b=>b.onclick=()=>{const q=currentPool.find(x=>x.label===b.dataset.review);audio.src=q.audio;audio.currentTime=0;audio.play().catch(()=>{$('save-status').textContent='複習聲音無法播放，請確認音量與網路。';});});
