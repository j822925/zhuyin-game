...
export function poolFor(mode,config,catalog) {
  if(mode==='single') return config.symbols.map(label=>({id:label,label,audio:'audio/audio_F'+(BASE.indexOf(label)+1)+'.WAV'}));
...
export function optionsFor(target,pool,count=3,rng=Math.random) {
  ...
  return shuffle([target,...shuffle(wrong,rng).slice(0,Math.max(0,count-1))],rng);
}
