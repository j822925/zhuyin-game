...
test('指南導向正式首頁，保留登入與需要網路的提醒',()=>{
 const guide=fs.readFileSync(new URL('install.html',root),'utf8');assert.match(guide,/href="\.\/\?v=20260913-home1"/);assert.match(guide,/打開為網頁 App/);assert.match(guide,/四位數密碼/);assert.match(guide,/需要網路/);assert.match(guide,/尚未用實體 iPad/);assert.match(guide,/不是這一頁教學/);
