...
test('前端預覽與每日上限一致，剩餘額度不足時只發剩餘數量',()=>{assert.equal(cappedRoundAward(2,2,'single',{perfectStars:2,perseveranceStars:2}).stars,0);const a=cappedRoundAward(1,3,'spelling',{perfectStars:11,perseveranceStars:4});assert.equal(a.perfectStars,1);assert.equal(a.perseveranceStars,1);assert.equal(a.stars,2);assert.equal(cappedRoundAward(2,2,'single',{perfectStars:2,perseveranceStars:2},true).stars,2);});
